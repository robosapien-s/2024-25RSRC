...
public class leftSideAuto extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        TrajectoryActionBuilder trajectory1 = drive.actionBuilder(initPose)
                ...
                .waitSeconds(0.5)
                .stopAndAdd(robotAuto.setHorizontalSlidePosition(RoboSapiensTeleOp.Params.HORIZONTAL_SLIDE_MAX_POSITION))
                .strafeToLinearHeading(new Vector2d(-61.5, -55), Math.toRadians(93.5))
                .waitSeconds(1)
                .stopAndAdd(robotAuto.openTopClawAction())
                .waitSeconds(.2)
//                .stopAndAdd(robotAuto.setState(IRobot.State.INTAKINGCLAW))
//                .waitSeconds(.2)
                .stopAndAdd(robotAuto.setState(IRobot.State.PICKUP_GROUND_LEFT)) //second
                .waitSeconds(2.5)
                .stopAndAdd(robotAuto.setState(IRobot.State.DROPPING_L2))
                .waitSeconds(2.5)
                .stopAndAdd(robotAuto.setHorizontalSlidePosition(1400))
                .stopAndAdd(robotAuto.openTopClawAction())
                .waitSeconds(.5)
                .strafeToLinearHeading(new Vector2d(-59, -51), Math.toRadians(108))
                .waitSeconds(.2)
                .stopAndAdd(robotAuto.setState(IRobot.State.PICKUP_GROUND_LEFT))//third
                .waitSeconds(1)
                .strafeToLinearHeading(new Vector2d(-61.5, -54), Math.toRadians(90))
                .waitSeconds(0.5)
                .stopAndAdd(robotAuto.setState(IRobot.State.DROPPING_L2))
                .waitSeconds(2.5)
                .stopAndAdd(robotAuto.openTopClawAction())
